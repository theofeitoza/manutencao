...
def buscar_dados_peca(nome_peca):
    ...
    conn = sqlite3.connect("manutencao.db")  # Substitua pelo seu banco de dados
    cursor = conn.cursor()
...
def salvar_ordens(equipamento, descricao_defeito, tipo_manutencao, equipe, criticidade, status):
    query = """
        INSERT INTO ordens (equipamento, descricao_defeito, tipo_manutencao, equipe, criticidade, status)
        VALUES (?, ?, ?, ?, ?, ?)
    """
    executar_query(banco_dados_unico, query, (equipamento, descricao_defeito, tipo_manutencao, equipe, criticidade, status), fetchall=False)
